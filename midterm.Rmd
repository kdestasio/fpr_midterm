...
```{r}
data_grouped <- data_sums %>% select(., -n) %>% 
...
# Example
gap(data_grouped[1:4, ], "white", "hispanic_latino")
## [1] -0.6091404
```
...
```{r}
filter(data_gap, grade_level == "Grade 6" | grade_level == "Grade 7" | grade_level == "Grade 8") %>% 
slice(., 1:100) %>% 
    ggplot(., aes(grade_level, achievement_gap)) +
    geom_()
```
