...
temp_first100 <- data_gap %>% slice(., 1:100) %>% 
    split(., .$school, drop = FALSE) %>%
    map(., ~achiev_plot(plot_data = .x, x = grade_level, y = achievement_gap, school = school, district = district, academic_year = academic_year))


achiev_plot <- function(plot_data, x, y, school, district, academic_year) {
ggplot(plot_data, aes(x = x, y = y, fill = y)) +
    geom_col(alpha = .5) +
    geom_hline(yintercept = 0,size = 3) +
    labs(x = "Grade",
         y = "Effect Size",
         title = paste("Achievement Gap Estimates:", plot_data$school),
         subtitle = "Students coded as White as compared to those coded as Hispanic/Latino",
         caption = paste0(plot_data$academic_year, " School year, ", plot_data$district, ", Oregon")) +
    theme_minimal(base_size = 12) +
    theme(plot.title = element_text(hjust = 0.5, size = 30),
          plot.subtitle = element_text(hjust = 0.5, size = 25),
          plot.caption = element_text(hjust = 1),
